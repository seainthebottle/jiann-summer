...
이 문서는 이 프로젝트(`jiann-summer`)에서 작업할 때 필요한 명령어, 아키텍처 정보, 그리고 일관된 개발을 위한 규칙과 가이드를 담고 있습니다. 이 파일의 내용은 `CLAUDE.md`와 `GEMINI.md`에 동일하게 유지됩니다.
...
앱은 `http://localhost:3020`에서 실행됩니다. 서버가 정적 파일도 서빙하므로 별도 프론트엔드 서버가 필요하지 않습니다.
...
### A. 시간 처리 원칙 (Time Handling)
서버와 클라이언트 간의 시간 불일치 문제를 방지하기 위해 다음 원칙을 반드시 준수합니다.

#### 서버 및 데이터베이스 (Server & Database)
...
### E. 테스트 및 명령어 실행 제한
- **로컬 테스트 실행 금지**: Gemini(또는 에이전트)는 사용자의 허가 없이 임의로 로컬 환경에서 서버를 구동하거나 브라우저 등을 활용한 로컬 테스트(동작 검증 등)를 직접 수행하지 않습니다. 테스트 검증 작업은 사용자에게 명시적으로 요청하거나 지침에 따라서만 수행합니다.

### F. 프로젝트 구조 (Project Structure)
...
- `CLAUDE.md` / `GEMINI.md`: 개발 원칙 및 프로젝트 구조 가이드 (본 문서)
